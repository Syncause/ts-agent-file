...
// Force ws to use the pure JS path. Some Next.js/Webpack builds partially shim
// optional native bufferutil, which breaks sends with `bufferUtil.mask is not a function`.
    ...
    const traces = Array.from(traceGroups.entries()).map(([traceId, spans]) => {
        ...
        const formattedSpans = spans
            ...
            .map(span => {
                ...
                // Add explicit caller info fields (extracted from attributes)
